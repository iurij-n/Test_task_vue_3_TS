...
export function useUserTable(options: UseUserTableOptions = {}) {
  ...
  const newUser = ref({ name: '', email: '', role: 'user' as Role, sendWelcomeEmail: true });
  ...
  const validateNewUserEmail = () => {
    ...
    else if (users.value.some(u => u.email === newUser.value.email)) newUserErrors.value.email = 'Email существует';
  ...
  const addNewUser = async () => {
    ...
    try {
      ...
      const newU: User = {
        id: newId, name: newUser.value.name, email: newUser.value.email, role: newUser.value.role,
        status: 'active', registrationDate: new Date().toISOString(), lastActivity: new Date().toISOString(),
        avatar: null, loginCount: 0, postsCount: 0, commentsCount: 0
      };
  ...
  const exportToCSV = () => {
    ...
    const rows = usersToExport.map(u => [
      u.id, u.name, u.email, getRoleLabel(u.role),
      u.status === 'active' ? 'Активен' : 'Неактивен', formatDate(u.registrationDate)
    ]);
  ...
  const generateMockUsers = (count: number): User[] => {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      registrationDate: new Date(2020, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)).toISOString(),
  ...
  const getRoleLabel = (role: Role) => ({ admin: 'Администратор', user: 'Пользователь', moderator: 'Модератор' }[role] || role);
  ...
  return {
    // State
    users, isLoading, isSaving, error,
    searchQuery, filterRole, filterStatus, dateFrom, dateTo,
    sortColumn, sortDirection, currentPage, pageSize, selectedUsers,
    editingUserId, editForm, showAddUserModal, showDetailsModal, selectedUser,
    newUser, newUserErrors,
    // Computed
    filteredAndSearchedUsers, paginatedUsers, totalPages, paginationStart, paginationEnd, visiblePages, isAllSelected, isNewUserValid,
    // Methods
    loadUsers, sortBy, goToPage, toggleSelectUser, toggleSelectAll, startEdit, cancelEdit, saveEdit,
    deleteUser, deleteSelectedUsers, toggleUserStatus, openAddUserModal, closeAddUserModal,
    validateNewUserName, validateNewUserEmail, addNewUser, openUserDetails, closeDetailsModal,
    exportToCSV, clearDateFilter, clearAllFilters, resetPage,
    // Utils
    getRoleLabel, formatDate, formatRelativeTime, getActivityClass, getDefaultAvatar, validateEmail
  };
}